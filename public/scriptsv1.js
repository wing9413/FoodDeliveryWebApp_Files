...
app.controller('searchCtrl', function($scope, $http, $filter, $location, $anchorScroll) {
var lat ;
var lng ;
var totalrestaurant ;
  ...
  function findrestaurant(){
    $http.get('list.json').then(function mySucces(response) {
	  ...
	  totalrestaurant = response.data.restaurant.length;
	  $scope.totalrestaurant =totalrestaurant;
    });
  }
...
//limit display if too many, load more button
  $scope.showbtn = true;
  $scope.totalDisplayed = 15;  
  $scope.loadmore = function(){
	if($scope.totalDisplayed > totalrestaurant){
	  $scope.showbtn = false;
	}else{
  ...
  $scope.distance = function(rlat,rlng){
	...
	$scope.estime = (((resDistance/40)*60)+20);
...
//onload
  search();
    ...
    $scope.checkDeliveryTime = function(open,close){
        var open = checkOpen(open,close);
        return open;
    };
    
    $scope.checkPriceRage = function(price){
        var sign;
        if(price == "low")
        {
             sign = "$";   
 	...
 	$http.get("list.json").then(function(response){
 		...
 		for(var i = 0; i < check.length; i++){
 			if(check[i].name==name){
 				...
 				var lat = string.lat;
 				var lng = string.lng;
 				var result = '{"lat":"' + lat + '", "lng": "' + lng + '", "name":"' + name + '","open":"'+openOrClose + '"}';
 				var tkm = new Date();
